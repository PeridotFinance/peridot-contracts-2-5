...
export const getComputeUnitPriceAndLimit = async (
    ...
    try {
        ...
        const autoYes =
            process.env.LZ_ASSUME_YES === '1' || process.env.CI === '1' || process.env.NON_INTERACTIVE === '1'
        if (autoYes) {
            console.warn(
                'LZ_ASSUME_YES/CI/NON_INTERACTIVE enabled — falling back to hardcoded TransactionCuEstimates without prompting.'
            )
        }
        const continueByUsingHardcodedEstimate = autoYes
            ? true
