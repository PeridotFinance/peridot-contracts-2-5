...
interface IMagma {
    ...
    function depositWMON(uint256 assets, address receiver, uint256 referralId) external returns (uint256 shares);
...
contract MagmaBoostedDelegate is PErc20Delegate {
    ...
    function _requestRedemption(uint256 shares) external {
        ...
        uint256 requestId = magmaVault.requestRedeem(shares, address(this), address(this));
    ...
    function _completeRedemption() external {
        ...
        uint256 assets = magmaVault.redeem(requestId, address(this), address(this));
    ...
    function _ensureLocalLiquidity(uint256 amount) internal view {
        uint256 localCash = super.getCashPrior();
        require(localCash >= amount, "insufficient buffer - admin must complete redemption");
    }
    ...
    function _depositToVault(uint256 amount) internal {
        ...
        uint256 shares = magmaVault.depositWMON(amount, address(this), referralId);
