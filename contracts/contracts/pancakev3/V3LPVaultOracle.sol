...
contract V3LPVaultOracle is PriceOracle, Ownable {
     ...
     */
    function setUsePoolDerivedPrices(address shareToken, bool enabled) external onlyOwner {
        VaultConfig storage cfg = vaultConfigs[shareToken];
        require(address(cfg.vault) != address(0), "vault missing");
    ...
    function _computeSharePrice(VaultConfig memory cfg) internal view returns (uint256) {
        ...
        if (usePoolDerivedPrices[address(cfg.vault)]) {
            ...
            uint256 poolRatio = _getTwapRatio(cfg); // token0/token1 ratio from pool
            // poolRatio is token1 per token0, so price0 = price1 / poolRatio
            price0 = poolRatio > 0 ? FullMath.mulDiv(STABLECOIN_PRICE, 1e18, poolRatio) : STABLECOIN_PRICE;
        } else {
