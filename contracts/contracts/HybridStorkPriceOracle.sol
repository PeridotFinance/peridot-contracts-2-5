...
contract HybridStorkPriceOracle is PriceOracle {
    ...
    /// @notice Optional per-asset external oracle (expects getPrice(address) -> 1e18 price).
    mapping(address => address) public externalAssetOracles;
    ...
    function getUnderlyingPrice(
        ...
        if (price == 0) {
            address ext = externalAssetOracles[asset];
            if (ext != address(0)) {
                try IAssetOracle(ext).getPrice(asset) returns (uint256 extPrice) {
                    price = extPrice;
                } catch {}
            }
        }
    ...
    function _getStorkPrice(address asset) internal view returns (uint256) {
        ...
        returns (StorkStructs.TemporalNumericValue memory value) {
            ...
            int192 qValue = value.quantizedValue;
            if (qValue < 0) {
                // negative values are not allowed
                return 0;
            }
            return uint256(uint192(qValue)); // quantized value already scaled to 18 decimals
        } catch {}

        return lastValidStorkPrice[asset];
    }
    ...
    /// @notice Set an external per-asset oracle (must implement getPrice(address) -> price 1e18).
    function setExternalOracle(address asset, address oracle) external onlyOwner {
        externalAssetOracles[asset] = oracle;
    ...
    function updateStorkCache(address[] calldata assets) external {
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            returns (StorkStructs.TemporalNumericValue memory value) {
                ...
                uint256 scaledValue = uint256(uint192(qValue));
                lastValidStorkPrice[asset] = scaledValue;
                emit LastStorkPriceUpdated(asset, scaledValue);
            } catch {
                continue;
            }
        }
